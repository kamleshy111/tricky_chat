...
if ( ! function_exists('online_operator')){
	function online_operator()
	{
		...
		WHERE users.last_activity > DATE_SUB('$now', INTERVAL $interval SECOND)");
		return $query;
	}
}
...
if ( ! function_exists('online_guest_count')){
	function online_guest_count()
	{
		...
		if(\Auth::user()->user_type == "admin" || get_option('allow_department') != "yes"){
			$query = DB::select("SELECT guests.id
			...
			WHERE guests.last_activity > DATE_SUB('$now', INTERVAL $interval SECOND) GROUP BY chat_requests.id");
			return count($query);
		}else{
			...
			FROM guests JOIN chat_requests ON guests.id= chat_requests.guest_id LEFT JOIN messages ON chat_requests.id=messages.chat_request_id 
			AND messages.status='0' AND messages.sender!='operator' LEFT JOIN users ON chat_requests.operator_id=users.id 
			WHERE guests.last_activity > DATE_SUB('$now', INTERVAL $interval SECOND) AND guests.department_id='$department_id' GROUP BY chat_requests.id");
			return count($query);
		}
	}
}
...
if ( ! function_exists('offline_message_count')){
	...
	{
		$count = \App\OfflineMessage::where('status','=','0')->count();
