...
$(document).ready(function () {
	...
	$(document).on('click','.btn-send',function(){
		if($("#chat_request_id").val() != "" && $(".chat-input-box").html() != ""){
			$.ajax({
				method: "POST",
				...
				success: function(data){
				  ...
				  if(json['result'] == 'send'){
					...
					toastr.error('Sorry You cannot send any message !');
				  }	
...
updateUserActivity = function (token) {
    $.ajax({
		...
		success: function(data){
			...
			//Show alert for new user join
			var new_users = json['new_users'];
			for(m = 0; m < new_users.length; m++){
				
				if(array_contains(newUser,new_users[m]['id']) == false){
					newUser.push(new_users[m]['id']);
					toastr.options.closeDuration = 2000;
					toastr.info('A new chat request has received from ' + new_users[m]['name']+'. Currently no operator has joined !', 'New Chat Request');
				    chatRequestSound();
				}
...
chatRequestSound = function(){
	var sound = document.getElementById("chatRequestSound"); 
	sound.play();
}
...
array_contains = function (arr, element) {
