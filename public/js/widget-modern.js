	...
	$.ajax({
			...
			success: function(data){
				...
				if (json['result'] = "success") {
					$(".panel-heading #operator_status").removeClass("offline-bnt");
					$(".panel-heading #operator_status").removeClass("online-bnt");
					$(".panel-heading #operator_status").addClass( json['data'] + "-bnt");
				}
	...
	if (input.files && input.files[0]) {
		 ...
		 $.ajax({
			 ...
			 beforeSend: function(){
				...
				$(".btn-fileupload").html('<i class="fas fa-circle-notch fa-spin"></i>'); 
			 },success: function(data){
				var json =JSON.parse(data);
				if(json['result'] == "success"){
				  //Show Uploaded File
...
//If screen is fullscreen	
$(window).on('load', function(e) {
	if($(".tricky_chat_widget").height() != 500){
		//$(".tricky_chat_widget .chat_area").css("height",$(window).height()-200);
	    $(".tricky_chat_widget .chat_area").css("height",$(window).height()-150);	
	}
});
